...
const heroWrapperVariants = createFadeUp(12, 0.08, 0.28);
const heroContentVariants = createStagger(0.08, 0.1);
const heroHeadingVariants = createFadeUp(20, 0, 0.42);
const heroCopyVariants = createFadeUp(16, 0, 0.34);
const heroActionVariants = createFadeUp(14, 0, 0.32);

export function HomeHero({ content, motionReady = true }: HomeHeroProps) {
  ...
  return (
    <section className="relative min-h-[760px] overflow-hidden bg-[#090909] pt-[72px] text-white">
      ...
      <div className="page-container relative flex min-h-[688px] items-center">
        <motion.div
          className="max-w-[58rem] pb-20 pt-10"
          initial={prefersReducedMotion ? false : "hidden"}
          animate={shouldAnimateEntrance ? "visible" : prefersReducedMotion ? undefined : "hidden"}
          variants={heroWrapperVariants}
        >
          <motion.div className="mt-10 space-y-8" variants={heroContentVariants}>
            <motion.h1
              className="font-sans text-[5.7rem] font-normal leading-[0.94] tracking-[-0.065em] text-white max-lg:text-[4.7rem] max-md:max-w-[22rem] max-md:text-[3.7rem] max-sm:text-[3rem]"
            ...
            <motion.div
              className="max-w-[48rem] space-y-2 text-[1rem] leading-[1.75] text-[#b0b5be] sm:text-[1.15rem]"
